...
class icerik_ureticisi(kisi):
    def __init__(self, isim, soyisim, kullanici_adi, sifre, yetki = "yazar"):
    ...
    def iceriklerimi_goruntule(self):
        dergiler = []
        konferanslar = []
        makaleler = []

        # Kullanıcının tam adı
        tam_isim = f"{self.isim_goster()} {self.soyisim_goster()}"
        # Dergiler dosyasını açıyoruz
        with open("dergiler.txt", "r", encoding="utf-8") as dosya:
            for satir in dosya:
                satir_verileri = [parca.strip() for parca in satir.strip().split(" - ")]
                if f"{satir_verileri[5]}" == tam_isim:
                    dergiler.append(" - ".join(satir_verileri))  

        with open("konferanslar.txt", "r", encoding="utf-8") as dosya:
            for satir in dosya:
                satir_verileri = [parca.strip() for parca in satir.strip().split(" - ")]
                if f"{satir_verileri[0]}" == tam_isim:
                    konferanslar.append(" - ".join(satir_verileri)) 
        with open("makaleler.txt", "r", encoding="utf-8") as dosya:
            for satir in dosya:
                satir_verileri = [parca.strip() for parca in satir.strip().split(" - ")]
                # İsim ve soyisim kontrolü
                if f"{satir_verileri[0]}" == tam_isim:
                    makaleler.append(" - ".join(satir_verileri))  
        print("Dergiler:")
        for dergi in dergiler:
            print(dergi)

        print("\nKonferanslar:")
        for konferans in konferanslar:
            print(konferans)
    ...
    def icerik_islemleri(self):
        ...
        with open("dergiler.txt", "r", encoding="utf-8") as dosya:
            for satir in dosya:
                satir_verileri = [parca.strip() for parca in satir.strip().split(" - ")]
                if f"{satir_verileri[5]}" == tam_isim:  # Tam isim kontrolü
                    icerikler.append(("Dergi", " - ".join(satir_verileri)))  # Tür ekleniyor ama satir_verileri değişmiyor

        # Konferanslar dosyasını açıyoruz
        with open("konferanslar.txt", "r", encoding="utf-8") as dosya:
            for satir in dosya:
                satir_verileri = [parca.strip() for parca in satir.strip().split(" - ")]
                if f"{satir_verileri[0]}" == tam_isim:  # Tam isim kontrolü
                    icerikler.append(("Konferans", " - ".join(satir_verileri)))  # Tür ekleniyor ama satir_verileri değişmiyor

        # Makaleler dosyasını açıyoruz
        with open("makaleler.txt", "r", encoding="utf-8") as dosya:
            for satir in dosya:
                satir_verileri = [parca.strip() for parca in satir.strip().split(" - ")]
                if f"{satir_verileri[0]}" == tam_isim:  # Tam isim kontrolü
                    icerikler.append(("Makale", " - ".join(satir_verileri)))  # Tür ekleniyor ama satir_verileri değişmiyor
